...
7. **Link blockers** from step 2 with GitHub's native dependencies (details in `docs/agents/issue-tracker.md`):
   ...
   ```
8. **Run the no-context review.** Dispatch a fresh general-purpose sub-agent with only this prompt: "Read GitHub issue #N in this repo with `gh issue view N`. Do not implement it. List every point where you would have to guess, look something up that the spec should have stated, or choose between interpretations. Return an empty list if there are none." Fix each gap in the issue body with `gh issue edit N --body-file <file>`, then dispatch a new sub-agent. Done when a sub-agent returns an empty list.
