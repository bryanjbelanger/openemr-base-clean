...
5. The job runs a Claude Code agent with the implement skill in its own worktree (P15). Hooks check each edit and block finishing until checks pass (P18).
6. The agent pushes a branch and opens a PR. A pre-push git hook runs the same checks (P18).
...
| P6 | Before labeling, a fresh sub-agent with only the issue and the repo lists anything it would have to guess. The author fixes gaps until the list is empty. | Directly tests the rule that a no-context agent can implement the spec. | A written checklist, which only tests that sections are filled in. |
...
| P15 | A new skill implements an issue from its spec in its own worktree, runs the checks, and opens a PR. | `piv-implement-issue` expects an RCA artifact that specs don't have, and the `piv-*` skills are vendored, so editing them conflicts with updates. | Modifying `piv-implement-issue`. |
...
| P20 | PHP 8.5 and Composer run on the Mac. Agent worktrees share one `vendor/` directory. | The edit hook must finish in about two seconds, and a container start per edit breaks that. PHP 8.5 is within the project's `>=8.2` requirement and is the version upstream CI tests most. | A throwaway PHP container per check. |
